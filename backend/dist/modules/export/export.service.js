...
let ExportService = class ExportService {
    constructor(dataSource) {
        ...
        this.resources = {
            products: {
                ...
                query: ({ q }) => {
                    const like = q ? `%${q}%` : '%';
                    return this.dataSource.query(`SELECT sku, name, category, brand, stock, min_stock AS "minStock",
            ...
            inventory: {
                ...
                query: ({ from, to, q }) => {
                    ...
                    const where = [
                        ...
                        .filter(Boolean)
                        .join(' AND ');
                    return this.dataSource.query(`SELECT m.created_at AS "createdAt", p.sku AS "productSku", p.name AS "productName",
            ...
            LIMIT 2000`, [like, from, to].filter((v) => v !== undefined && v !== null));
                },
            },
            sales: {
                title: 'Ventas',
                ...
                query: ({ from, to, q }) => {
                    ...
                    const where = [
                        ...
                        .filter(Boolean)
                        .join(' AND ');
                    return this.dataSource.query(`SELECT d.doc_number AS "docNumber", d.doc_type AS "docType", d.created_at AS "createdAt",
            ...
            customers: {
                ...
                query: ({ q }) => {
                    const like = q ? `%${q}%` : '%';
                    return this.dataSource.query(`SELECT code, name, document_type AS "documentType", document_number AS "documentNumber",
            ...
            suppliers: {
                ...
                query: ({ q }) => {
                    const like = q ? `%${q}%` : '%';
                    return this.dataSource.query(`SELECT code, name, tax_id AS "taxId", email, phone, address
             FROM suppliers
            ...
            purchases: {
                ...
                query: ({ from, to, q }) => {
                    ...
                    const where = [
                        ...
                        .filter(Boolean)
                        .join(' AND ');
                    return this.dataSource.query(`SELECT d.doc_number AS "docNumber", d.created_at AS "createdAt", s.name AS "supplierName",
            ...
            employees: {
                ...
                query: ({ q }) => {
                    const like = q ? `%${q}%` : '%';
                    return this.dataSource.query(`SELECT code, full_name AS "fullName", position, department, phone, email,
                  hire_date AS "hireDate", salary
             FROM employees
            WHERE (full_name ILIKE $1 OR code ILIKE $1 OR position ILIKE $1 OR email ILIKE $1)
            ORDER BY full_name`, [like]);
                },
            },
            audit: {
                title: 'Auditoría',
                ...
                query: ({ from, to, q }) => {
                    ...
                    const where = [
                        ...
                        .filter(Boolean)
                        .join(' AND ');
                    return this.dataSource.query(`SELECT a.created_at AS "createdAt", u.full_name AS "userFullName", a.action,
    ...
    async export(resource, format, params) {
        ...
        if (!def)
            throw new common_1.BadRequestException(`Recurso de exportación no válido: ${resource}`);
        if (!['csv', 'xlsx', 'pdf'].includes(format)) {
            throw new common_1.BadRequestException(`Formato no válido: ${format}`);
        }
    ...
    async toExcel(columns, rows, title) {
    ...
    async toPdf(columns, rows, title) {
